...
class Controller:
    ...
    def __init__(self,ttyStr='/dev/ttyACM99',device=0x0c): #std: /dev/ttyACM0
    ...
    def setRange(self, chan, min, max):
        self.Mins[chan] = min
        self.Maxs[chan] = max
    def setOffset(self,chan,wert):
        self.offset[chan] = wert
    # Return Minimum channel r/deve
    def getMin(self, chan):
        return self.Mins[chan]

    # Return Maximum channel r/deve
    ...
    # Servo center is at 1500 microseconds, 
    # or 6000 quarter-microseconds//////// deshalb target = target * 4
    # Typcially valid servo range is 3000 to 9000 quarter-microseconds
    # If channel is configured for digital output, values < 6000 = Low ouput
    def setTarget(self, chan, steuerung):
        if (steuerung > 0):
            if (steuerung > 1):
                steuerung = 1
            steuerung = (self.Maxs[chan] - 1500 - self.offset[chan]) * steuerung + self.offset[chan]
        else:
            if (steuerung < -1):
                steuerung = -1
            steuerung = (abs(self.Mins[chan] - 1500 - self.offset[chan])) * steuerung + self.offset[chan]
        target = 1500 + steuerung
